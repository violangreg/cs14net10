...
public static class PrimeFactorsLib
{
    ...
    {
        string? validate = number switch
        {
            < 1 => $"Number {number} is out of range (1-1000)",
            1 => "None",
            > 1000 => $"Number {number} is out of range (1-1000)",
            _ => null,
        };
        ...
        return result[..^3]; // Modern way to Substring(0, input.Length - 3) === Remove the last 3 characters (" x ")
    }
}
