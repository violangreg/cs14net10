partial class Program
{
    ...
    static void FilteringUsingWhere(string[] names)
    {
        ...
        // deferred -- explicitly creating the required delegate
    ...
    static void WorkingWithIndices()
    {
        ...
        SectionTitle("Working With Indices (.NET 8 and earlier)");
        foreach (var (item, index) in theSeven.Select((item, index) => (item, index)))
        {
            WriteLine($"{index}: {item}");
        }
        SectionTitle("Working With Indices (.NET 9 and later)");
